...
class BenchmarkEvaluator:
    ...
    # ─── PROMPT FORMATTING (FIXED - Simple Format) ─────────────────────────
    ...
    def format_prompt(self, question: str, dataset: str, tokenizer) -> str:
        ...
        Uses simple text format instead of chat template for reliability.
        """
        ...
        # SIMPLE FORMAT - No chat template (more reliable)
        return f"<|system|>\n{system}\n<|user|>\nProblem: {question}\n\nSolve step by step:\n<|assistant|>\n"
    ...
    def extract_model_answer(self, response_text: str) -> str:
        ...
        # Priority 5: yes/no anywhere (before numbers)
        yn2 = re.search(r'\b(yes|no)\b', text, re.IGNORECASE)
        if yn2:
            return yn2.group(1).lower()

        # Priority 6: single letter A-D anywhere
        let = re.search(r'\b([A-Da-d])\b', text)
        if let:
            return let.group(1).upper()

        # Priority 7: last number anywhere (fallback for GSM8K and numeric baselines)
    ...
    # ─── MODEL GENERATION (FIXED - Greedy Decoding) ────────────────────────────────────
    ...
    def evaluate_checkpoint(
        ...
        if model_type == "hf":
            ...
            if is_peft:
                ...
                # Load tokenizer from the adapter path (or base model)
                tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=False)
                if tokenizer.pad_token is None:
                    tokenizer.pad_token = tokenizer.eos_token
            else:
                # Load full model directly
                logger.info("Loading full model from path...")
                bnb = BitsAndBytesConfig(
                    load_in_4bit=True, 
                    bnb_4bit_compute_dtype=torch.bfloat16,
                    bnb_4bit_use_double_quant=True,
        ...
        for idx, sample in enumerate(samples):
            ...
            # GSM8K fix: if extraction returned "yes"/"no", extract last number
    ...
    def compare_baseline_sft_craft(self, baseline_path: str, sft_path: str, craft_path: str, num_samples: int = 100) -> dict:
        """Compare baseline, SFT, and CRAFT models."""
        datasets = ["gsm8k", "strategyqa", "mmlu"]
        results = {}
        
        print("\n" + "=" * 80)
        print("CRAFT vs Baseline vs SFT Comparison")
        print("=" * 80)
        ...
        for dataset in datasets:
            ...
            baseline = self.evaluate_checkpoint(baseline_path, dataset, num_samples, "baseline", "hf")
            sft = self.evaluate_checkpoint(sft_path, dataset, num_samples, "sft", "hf")
            craft = self.evaluate_checkpoint(craft_path, dataset, num_samples, "craft", "hf")
            ...
            print(f"  Baseline: {baseline['summary']['accuracy_pct']:.1f}%")
            print(f"  SFT:      {sft['summary']['accuracy_pct']:.1f}%")
            print(f"  CRAFT:    {craft['summary']['accuracy_pct']:.1f}%")
            print(f"  Improvement: +{(craft['summary']['accuracy_pct'] - baseline['summary']['accuracy_pct']):.1f}%")
...
def main():
    ...
    parser.add_argument("--model-path", required=True, help="Path to model or checkpoint")
    ...
    parser.add_argument("--output-json", default="craft_output/results.json", help="Output JSON file")
    parser.add_argument("--compare", action="store_true", help="Compare baseline, SFT, and CRAFT")
    parser.add_argument("--baseline-path", default="microsoft/Phi-3-mini-4k-instruct", help="Baseline model path")
    parser.add_argument("--sft-path", default="checkpoints/sft/final", help="SFT model path")
    parser.add_argument("--craft-path", default="checkpoints/rl/final", help="CRAFT model path")
    ...
    if args.compare:
        results = evaluator.compare_baseline_sft_craft(
            args.baseline_path, args.sft_path, args.craft_path, args.samples
        )
        with open(args.output_json, "w") as f:
            json.dump(results, f, indent=2)
        logger.info(f"Comparison results saved to {args.output_json}")
    else:
